...
export default function UserManagementPage() {
  ...
  const toggleLockStatus = async (userId: string, currentStatus: boolean) => {
    try {
      ...
      alert(`사용자가 성공적으로 ${!currentStatus ? "잠금" : "잠금 해제"}되었습니다`);
      fetchUsers(); // 목록 새로고침
    } catch (err: any) {
                ...
                {users.length === 0 ? (
                  ...
                  users.map((user) => (
                    <tr key={user.id} className="hover:bg-gray-50">
                      ...
                      <td className="py-2 px-4 border">{translateStatus(user.status)}</td>
                      <td className="py-2 px-4 border">
                        <span
...
// 상태 텍스트 번역 함수
function translateStatus(status?: 'online' | 'offline' | 'busy' | 'away'): string {
  switch (status) {
    case 'online':
      return '온라인';
    case 'offline':
