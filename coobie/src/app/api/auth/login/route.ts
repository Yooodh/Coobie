...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // 인증 시도
      const { user: authenticatedUser, token } = await authenticateUserUseCase.execute(
        username,
        password
      );
      ...
      if (error.message === "USER_NOT_FOUND") {
        ...
        // 로그인 시도 횟수 증가
        const loginAttempts = await userRepository.incrementLoginAttempts(user.id);
        console.log(`사용자 ${username}의 로그인 시도 횟수: ${loginAttempts}/${MAX_LOGIN_ATTEMPTS}`);
