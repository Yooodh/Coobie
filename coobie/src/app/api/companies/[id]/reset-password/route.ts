// src/app/api/companies/[id]/reset-password/route.ts (수정됨)
...
export async function POST(
  ...
  try {
    ...
    const resetCompanyPasswordUseCase = new ResetCompanyPasswordUseCase(companyRepository);

    // 비밀번호 초기화 및 계정 잠금 해제
    await resetCompanyPasswordUseCase.execute(id, defaultPassword);
    
    return NextResponse.json({
      message: `ID가 ${id}인 회사의 관리자 비밀번호가 성공적으로 초기화되었습니다`
    });
  } catch (error: any) {
    console.error(
      `회사 관리자 비밀번호 초기화 중 오류 발생 (ID: ${id}): `,
      error
    );
    return NextResponse.json(
