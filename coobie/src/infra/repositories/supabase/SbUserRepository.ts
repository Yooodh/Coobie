...
export class SbUserRepository implements UserRepository {
  ...
  async findAll(filter?: UserFilter): Promise<User[]> {
    ...
    let query = supabase.from("user").select("*").is("deleted_at", null);
  ...
  async findByRoleId(roleId: string, filter?: UserFilter): Promise<User[]> {
    ...
    let query = supabase.from("user").select("*").eq("role_id", roleId);
  ...
  async update(user: User): Promise<User> {
    ...
    if (user.notificationOn !== undefined)
      updates.notification_on = user.notificationOn;
